...
def configure(keymap):
    ...
    other_window_key = False

    # クリップボードリストを起動するキーを指定する
    clipboardList_key = False

    # ランチャーリストを起動するキーを指定する
    lancherList_key = False
    ...
    define_key(keymap_emacs, "C-Left",        reset_search(reset_undo(reset_counter(mark(move_beginning_of_line)))))
    define_key(keymap_emacs, "C-e",        reset_search(reset_undo(reset_counter(mark(move_end_of_line)))))
    define_key(keymap_emacs, "C-Right",        reset_search(reset_undo(reset_counter(mark(move_beginning_of_line)))))
